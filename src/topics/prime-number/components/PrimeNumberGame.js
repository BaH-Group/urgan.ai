      ...
      <div className="flex flex-col items-center justify-center w-full mb-4">
        {currentPuzzle.type === 'prime_is_prime' && (
          <div className="flex flex-col items-center gap-6 mb-4">
            <div className="text-6xl md:text-8xl font-black text-slate-700 bg-slate-50 p-8 rounded-3xl border border-slate-200 shadow-sm min-w-[180px] text-center">
            ...
            <div className="flex gap-4 w-full max-w-xs">
              {currentPuzzle.options.map((opt, idx) => (
                <button 
                  ...
                  className={`flex-1 py-4 md:py-6 rounded-2xl text-2xl md:text-3xl font-black transition-all hover:-translate-y-1 hover:shadow-lg border-4 
          ...
          <div className="grid grid-cols-2 gap-3 md:gap-4 w-full max-w-sm mb-4">
            {currentPuzzle.options.map((opt, idx) => (
              <button 
                key={`${opt}-${idx}`}
                onClick={() => handleAnswer(idx)}
                className="py-6 md:py-8 bg-white border-4 border-slate-200 hover:border-sky-400 rounded-2xl text-3xl md:text-4xl font-black text-slate-700 transition-all hover:-translate-y-1 hover:shadow-md"
              >
                {opt}
              </button>
            ))}
        ...
        {(currentPuzzle.type === 'prime_next' || currentPuzzle.type === 'prime_smallest_factor') && (
          <div className="flex flex-col items-center w-full max-w-md bg-slate-50 p-6 md:p-8 rounded-3xl border border-slate-200 shadow-sm mb-4">
            <div className="text-5xl md:text-7xl font-black text-slate-700 mb-6">
            ...
            <div className="mt-6 w-full transform scale-90 md:scale-100">
              <CustomKeypad />
            </div>
          </div>
        )}
